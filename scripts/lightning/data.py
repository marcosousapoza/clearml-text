...
class RelbenchLightningDataModule(L.LightningDataModule):
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        # PyG neighbor sampling expands *into* the current frontier. For label
        # seeds that means hop 1 must allow object -> label so the sampler can
        # fetch the owning object. After that first hop, however, we must block
        # any further expansion back into label nodes; otherwise an object can
        # pull in other label timestamps for the same entity and leak targets.
        num_neighbors = {}
        for et in data.edge_types:
            src, _, dst = et
            if dst.endswith("_labels"):
                num_neighbors[et] = [-1] + [0] * (self.num_layers - 1)
            elif src.endswith("_labels"):
                num_neighbors[et] = [0] * self.num_layers
            else:
                num_neighbors[et] = hop_counts
        self._loader_dict = {}
